...
def fetch_jobs() -> list[dict]:
    ...
    if not prefs.get("allow_remote") or prefs.get("remote_scope") == "none":
        return []
    roles = config.profile().get("target_roles") or []

    seen: set[str] = set()
    out: list[dict] = []
    for role in (roles[:MAX_ROLES] or [""]):
        try:
            r = SESSION.get(BASE, params={"search": role, "limit": LIMIT}, timeout=DEFAULT_TIMEOUT)
            r.raise_for_status()
        except requests.RequestException as e:
            log.warning("Remotive '%s' failed: %s", role, e)
            continue
